...
void Simulation::writeVTKFile(std::string fname_root, size_t tstep, size_t data_flags)
{
	...
	// This safe, if slightly less efficient, method ensures consistency.
...
/*
This doesn't have the overlapping
void Simulation::writeVTKFile(std::string fname_root, size_t tstep, size_t data_flags)
{
	// 1. Write the master .pvti file from the root process
	if (mpi_rank == 0)
	{
		stringstream fname;
		fname << fname_root << "_" << setw(6) << setfill('0') << tstep << ".pvti";
		...
		// --- DEFINITIVE FIX: Use the verified 'all_local_domains' array with correct, non-overlapping extents ---
		for (int proc = 0; proc < mpi_comm_size; ++proc)
		{
			stringstream piece_fname;
			string root_basename = fname_root.substr(fname_root.find_last_of("/\\") + 1);
			...
			fout << "\t\t<Piece Extent=\"";
			fout << piece_dom.origin.i << " " << piece_dom.origin.i + piece_dom.extent.i - 1 << " ";
			fout << piece_dom.origin.j << " " << piece_dom.origin.j + piece_dom.extent.j - 1 << " ";
			fout << piece_dom.origin.k << " " << piece_dom.origin.k + piece_dom.extent.k - 1 << "\" ";
			fout << "Source=\"" << piece_fname.str() << "\"/>" << endl;
		}
	...
	// 2. Each process writes its own .vti piece file (consistent and non-overlapping)
	stringstream fname;
	fname << fname_root << "_" << mpi_rank << "_" << setw(6) << setfill('0') << tstep << ".vti";

	vtkSmartPointer<vtkImageData> imageData = vtkSmartPointer<vtkImageData>::New();
	imageData->SetExtent(local.origin.i, local.origin.i + local.extent.i - 1,
						 local.origin.j, local.origin.j + local.extent.j - 1,
						 local.origin.k, local.origin.k + local.extent.k - 1);
	...
	if (data_flags & VTKOutput::Type)
	{
		...
		arr->SetName("MaterialType");
		arr->SetArray(img_data.getData().get() + img_data.pad_size, local.extent.size(), 1);
		imageData->GetPointData()->AddArray(arr);
#elif DATA_TYPE == 8
	...
	if (data_flags & VTKOutput::Conc)
	{
		auto arr = vtkSmartPointer<vtkDoubleArray>::New();
		arr->SetName("Concentration");
		arr->SetArray(conc.getData().get() + conc.pad_size, local.extent.size(), 1);
		imageData->GetPointData()->AddArray(arr);
	}
	if (data_flags & VTKOutput::Pressure)
	{
		auto arr = vtkSmartPointer<vtkDoubleArray>::New();
		arr->SetName("Pressure");
		arr->SetArray(pressure.getData().get() + pressure.pad_size, local.extent.size(), 1);
		imageData->GetPointData()->AddArray(arr);
	}
	if (data_flags & VTKOutput::Frac)
	{
		auto arr = vtkSmartPointer<vtkFloatArray>::New();
		arr->SetName("SulphideFrac");
		arr->SetArray(frac.getData().get() + frac.pad_size, local.extent.size(), 1);
		imageData->GetPointData()->AddArray(arr);
	}
	if (data_flags & VTKOutput::Flux_Vec)
	{
		...
		arr->SetName("Flux");
		arr->SetNumberOfComponents(3);
		arr->SetNumberOfTuples(local.extent.size());
		for (long long i = 0; i < local.extent.size(); ++i)
		{
			arr->SetTuple3(i, flux_x[i], flux_y[i], flux_z[i]);
		}
		imageData->GetPointData()->SetVectors(arr);
	}
...
void Simulation::writeVTKFile(std::string fname_root, size_t tstep, size_t data_flags)
{
	...
	if (mpi_rank == 0)
	{
		...
		// --- DEFINITIVE FIX: Replicate the original code's working overlap logic ---
		for (int proc = 0; proc < mpi_comm_size; ++proc)
		{
			stringstream piece_fname;
			string root_basename = fname_root.substr(fname_root.find_last_of("/\\") + 1);
	...
	// 2. Each process writes its own .vti piece file (This part is correct and non-overlapping)
	stringstream fname;
	fname << fname_root << "_" << mpi_rank << "_" << setw(6) << setfill('0') << tstep << ".vti";
...
void Simulation::writeVTKFile(std::string fname_root, size_t tstep, size_t data_flags)
{
	...
	if (mpi_rank == 0)
	{
		...
		int block_size_i = global.extent.i / mpi_comm_size;
		for (int proc = 0; proc < mpi_comm_size; ++proc)
		{
			stringstream piece_fname;
			string root_basename = fname_root.substr(fname_root.find_last_of("/\\") + 1);
			...
			int origin_i = proc * block_size_i;
			int extent_i = (proc == mpi_comm_size - 1) ? (global.extent.i - origin_i) : block_size_i;

			fout << "\t\t<Piece Extent=\"";
			fout << origin_i << " " << origin_i + extent_i - 1 << " ";
			fout << "0 " << global.extent.j - 1 << " ";
			fout << "0 " << global.extent.k - 1 << "\" ";
			fout << "Source=\"" << piece_fname.str() << "\"/>" << endl;
		}
	...
	// 2. Each process writes its own .vti piece file
	stringstream fname;
	fname << fname_root << "_" << mpi_rank << "_" << setw(6) << setfill('0') << tstep << ".vti";
	...
	if (data_flags & VTKOutput::Type)
	{
		// --- FIX: Moved logic inside the preprocessor blocks to fix scope issue ---
#if DATA_TYPE == 16
		auto arr = vtkSmartPointer<vtkUnsignedShortArray>::New();
		arr->SetName("MaterialType");
		arr->SetArray(img_data.getData().get() + img_data.pad_size, local.extent.size(), 1);
...
void Simulation::writeVTKFile(std::string fname_root, size_t tstep, size_t data_flags)
{
	// 1. Write the master .pvti file from the root process (This part is now correct)
	if (mpi_rank == 0)
	{
		stringstream fname;
		fname << fname_root << "_" << setw(6) << setfill('0') << tstep << ".pvti";
	...
	// --- START FIX: Ensure piece files are written consistently with the manifest ---
	// 2. Each process writes its own .vti piece file, including the overlap/ghost cell
	stringstream fname;
	fname << fname_root << "_" << mpi_rank << "_" << setw(6) << setfill('0') << tstep << ".vti";
	...
	// Calculate the number of voxels to write, including the overlap
	size_t num_voxels_to_write = (size_t)(local.extent.i + off_pos) * local.extent.j * local.extent.k;
	...
	if (data_flags & VTKOutput::Type)
	{
		...
		arr->SetName("MaterialType");
		arr->SetNumberOfValues(num_voxels_to_write);
		// Note: The loop below now handles the copy
		imageData->GetPointData()->AddArray(arr);
#elif DATA_TYPE == 8
		auto arr = vtkSmartPointer<vtkUnsignedCharArray>::New();
		arr->SetName("MaterialType");
		...
		imageData->GetPointData()->AddArray(arr);
#endif
		// Doing this here fails at compile (arr not defined)
		// So moved to the #if condition
		// arr->SetName("MaterialType");
		// arr->SetNumberOfValues(num_voxels_to_write);
		//// Note: The loop below now handles the copy
		// imageData->GetPointData()->AddArray(arr);
	...
	// ... (Similar setup for Pressure, Frac, etc.)

	// Loop over the region to be written, including the overlap, and copy data point-by-point
	size_t count = 0;
	for (int k = local.origin.k; k < (local.origin.k + local.extent.k); ++k)
	{
		for (int j = local.origin.j; j < (local.origin.j + local.extent.j); ++j)
		{
			for (int i = local.origin.i; i < (local.origin.i + local.extent.i + off_pos); ++i)
			{
				...
				if (data_flags & VTKOutput::Conc)
					imageData->GetPointData()->GetArray("Concentration")->SetTuple1(count, conc[idx]);
				// ... (Add similar lines for Pressure, Frac, etc.)
				count++;
			}
		}
	}

	// Special handling for 3-component flux vector
	if (data_flags & VTKOutput::Flux_Vec)
	{
		auto arr = vtkSmartPointer<vtkDoubleArray>::New();
		arr->SetName("Flux");
		...
		for (int k = local.origin.k; k < (local.origin.k + local.extent.k); ++k)
		{
			for (int j = local.origin.j; j < (local.origin.j + local.extent.j); ++j)
			{
				...
				{
					Index idx(i, j, k);
					arr->SetTuple3(vec_count, flux_x[idx], flux_y[idx], flux_z[idx]);
					vec_count++;
				}
			}
		}
